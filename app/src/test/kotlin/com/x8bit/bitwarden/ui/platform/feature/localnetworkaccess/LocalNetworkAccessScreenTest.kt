...
class LocalNetworkAccessScreenTest : BitwardenComposeTest() {
    ...
    fun `screen displays title, description, and buttons`() {
        ...
        composeTestRule
            .onNodeWithText(
                text = "Bitwarden needs access to local networks in order to connect and sync" +
                    " with your server. This permission is required to continue.",
            )
            .assertIsDisplayed()
        composeTestRule
    ...
    fun `Enable local network access click with permission denied shows dialog`() {
        ...
        composeTestRule
            .onNodeWithText(
                text = "Local network access is required for communicating with your local server",
            )
            .assertIsDisplayed()
    }
